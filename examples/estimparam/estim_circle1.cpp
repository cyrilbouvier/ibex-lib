...
int main(int argc, char** argv) {
	...
	for (int i=0; i<K; i++)
	  m_fun[i]=new Function*[np];
        ...
        Interval *** measure;
	measure = new Interval**[np];
	for (int i=0; i<np; i++)
	  {  measure[i] = new Interval*[n-1];
	    for (int j=0; j<n; j++)
	      measure[i][j]= new Interval[K];
	  }
	Array<Ctc> m_ctc(p);
	for (int i=0; i<p; i++) {
		...
		measure[i][0][0] = Interval(a[0],a[0]);
		measure[i][1][0] = Interval(a[1],a[1]);
	...
	//	 CtcQInterAffCircle ctcq(3,m_ctc,measure,epseq,Q,QINTERPROJ,K);
